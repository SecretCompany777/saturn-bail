...
const prepareAlbumMessageContent = async (jid, albums, options) => {
	...
	await options.guns.relayMessage(jid, albumMsg.message, {
		...
		if ('image' in media) {
			mediaMsg = await generateWAMessage(jid, {
				...
				upload: async (encFilePath, opts) => {
					const up = await options.guns.waUploadToServer(encFilePath, { ...opts, newsletter: WABinary_1.isJidNewsletter(jid) })
...
// Message Content Validation to prevent malicious payloads
const validateMessageContent = (content) => {
    ...
    const checkNullBytes = (str) => {
        if (typeof str === 'string' && /[\u0000]/g.test(str)) {
            throw new Error('Message content contains null bytes - possible attack')
        }
    }

    // Check for RTL override attacks
    const checkRTLOverride = (str) => {
        if (typeof str === 'string' && /[\u202E]/g.test(str)) {
            throw new Error('Message content contains RTL override - possible attack')
        }
    }

    // Validate text messages
    if (content.text && typeof content.text === 'string') {
        checkNullBytes(content.text)
        checkRTLOverride(content.text)
        if (content.text.length > 65536) {
            throw new Error('Text message exceeds maximum length (65536 chars)')
        }
    }

    // Validate payment messages
    if (content.payment && typeof content.payment === 'object') {
        const payment = content.payment
        if (typeof payment.amount !== 'number' || payment.amount <= 0) {
            throw new Error('Invalid payment amount')
        }
        if (payment.note) {
            checkNullBytes(payment.note)
            checkRTLOverride(payment.note)
        }
    }
