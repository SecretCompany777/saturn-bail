...
const makeCommunitiesSocket = (config) => {
    const guns = business_1.makeBusinessSocket(config)
    const { authState, ev, query, groupMetadata, upsertMessage } = guns
    ...
    const communityFetchAllParticipating = async () => {
        ...
        guns.ev.emit('groups.update', Object.values(data))
        return data
    }
    
    guns.ws.on('CB:ib,,dirty', async (node) => {
    	...
    	await guns.cleanDirtyBits('groups') 
    }) 
