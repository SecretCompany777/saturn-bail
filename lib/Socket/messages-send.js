...
const makeMessagesSocket = (config) => {
    ...
    const saturn = new secret(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = saturn.detectType(content);
            ...
            if (messageType) {
                switch(messageType) {
                    case 'PAYMENT':
                        const paymentContent = await saturn.handlePayment(content, quoted);
                    ...
                    case 'PRODUCT':
                        const productContent = await saturn.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE':
                        const interactiveContent = await saturn.handleInteractive(content, jid, quoted);
                    ...
                    case 'ALBUM':
                        return await saturn.handleAlbum(content, jid, quoted)
                    case 'EVENT':
                        return await saturn.handleEvent(content, jid, quoted)
                    case 'POLL_RESULT':
                        return await saturn.handlePollResult(content, jid, quoted)
                    case 'GROUP_STORY':
                        return await saturn.handleGroupStory(content, jid, quoted)
                }
            }
