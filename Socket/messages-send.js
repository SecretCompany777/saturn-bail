...
const makeMessagesSocket = (config) => {
    ...
    const guns = newsletter_1.makeNewsletterSocket(config)
    const { ev, authState, processingMutex, signalRepository, upsertMessage, createCallLink, query, fetchPrivacySettings, sendNode, groupQuery, groupMetadata, groupToggleEphemeral, newsletterWMexQuery, executeUSyncQuery } = guns   
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' &&
                ...
                const isPayment = 'payment' in content && !!content.payment
                ...
                // Handle AI message with bot node
                if (isAi && !WABinary_1.isJidGroup(jid)) {
                ...
                // Validate payment message structure
                if (isPayment && typeof content.payment === 'object') {
                    const payment = content.payment
                    if (!payment.amount || typeof payment.amount !== 'number') {
                        throw new Error('Payment message requires valid amount (number)')
                    }
                    if (payment.amount <= 0) {
                        throw new Error('Payment amount must be greater than 0')
                    }
                    // Prevent malicious null/special chars in payment fields
                    if (payment.note && typeof payment.note === 'string' && /[\u0000\u202E]/g.test(payment.note)) {
                        throw new Error('Payment note contains invalid characters')
                    }
                }
                ...
                await relayMessage(jid, fullMsg.message, { messageId: fullMsg.key.id, useCachedGroupMetadata: options.useCachedGroupMetadata, additionalAttributes, statusJidList: options.statusJidList, additionalNodes: options.additionalNodes, AI: isAi || options.ai })
