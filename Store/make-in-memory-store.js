...
const makeInMemoryStore = (config) => {
    ...
    return {
        ...
        fetchImageUrl: async (jid, guns) => {
            const contact = contacts[jid]
            if (!contact) {
                return guns?.profilePictureUrl(jid)
            }
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await guns?.profilePictureUrl(jid)
            }
            return contact.imgUrl
        },
        fetchGroupMetadata: async (jid, guns) => {
            if (!groupMetadata[jid]) {
                const metadata = await guns?.groupMetadata(jid)
                if (metadata) {
